...
// mask - numpy array of shape=(height,width), dtype=uint8, 0-background, 255-foreground
// expand - (optional) expand borders by 1 pixel if true (1-pix lines -> 3-pix lines).
// Simplified postprocess_particle_borders() -- don't fill gaps, just make borders 1-pix (expand=False)
//		or 3-pix (expand=True) thick.
void refine_particle_borders(unsigned char *mask, int hm, int wm, bool expand=false);
...
//
// mask - numpy array (shape=[hm,wm] dtype=uint8 containing refined REShAPE segmentation (cell borders)
// data - numpy array of the same shape as 'mask' but of type uint16; it will receive the output ID mask
// in_csv - CSV file containing XStart,YStart pixel coordinates of each cell (as generated by Fiji particle analysis)
void create_id_mask(unsigned char *mask, int hm, int wm, unsigned short *data, int hd, int wd, const char *in_csv);

// imagetools.id_mask_merge_cells(data3d)
//		Try to merge cell objects separated due to missing frames in the middle
//
// data3d - on input, a numpy array (shape=[zd3d,hd3d,wd3d], dtype=uint16) containing cell IDs;
//		on output, same format, but may be fewer cell IDs due to the merging
// maxgap - (optional) int, maximum number of missing z-slices between IDs to merge
